...
In this case Equation \eqref{euler} is known as the Liouville equation \citep[e.g.][]{munk1960rotation}.
...
If we plug this decomposition into Equation \eqref{quasistatic_liouville} the spherically symmetric part $I_0 \delta_{ij}$ drops out, and we are left with
...
The infinite-time limit of Equation \eqref{viscoelastic_deformation} allows us to calculate the fluid limit of the Love number $k_f$:
\begin{equation}
...
\citet{ricard1993polar} obtain an approximation to Equation \eqref{viscoelastic_deformation} which retains its long-time behavior by entering the Laplace domain and truncating a Taylor series for $k(s)$ to first order.  
...
Since the first term represents the fluid limit of rotational deformation, it automatically satisfies Equation \eqref{quasistatic_liouville}, and hence does not contribute to the polar wander problem.

\subsection{The convective moment of inertia}
\label{sec:convective_moment}

The term on the right-hand side of equation \eqref{disequilibrium} represents the moment of inertia due to internal density anomalies as well as the surface deflections due to them.
...
Substituting Equation \eqref{rotational_deformation} into Equation \eqref{disequilibrium} we find
...
Introducing a unit vector $\mitbf{\omega} = {\bf \Omega}/\|{\bf \Omega} \|$ and using Equation \eqref{fluid_love} for $k_f$,  we may solve this equation for ${\bf \dot{\mitbf{\omega}} }$:
\begin{equation}
...
In order to estimate $\theta$ and $\lambda_i$ we must examine the dynamics of convection in the planetary interior.
...
\label{energy}
\end{equation}
here the parameters are defined in Table~\eqref{parameters}.
...
The other parameters are defined in Table \eqref{parameters}.
...
Convenient choices for these numbers are listed in Table \eqref{nondim}, along with approximate Earth-like values for them.
...
By definition the hydrostatic reference state is a solution to \eqref{navier_stokes} where there is no flow:
...
Nondimensionalizing with the parameters in Table \eqref{nondim} and removing the reference state we find the nondimensional momentum equation:
\begin{equation}
\begin{aligned}
...
Equation \eqref{navier_stokes} can be transformed into an angular momentum equation by crossing with $\bf r$ and integrating over the volume of the mantle:
...
Splitting the volume into $V_H$ and $\Delta V$ and substituting \eqref{eos} we find
...
This equation may be identified with \eqref{disequilibrium}, where disequilibrium in the rotational deformation (left side) is balanced by a the mismatch of the convective moment of intertia with the spin axis (right side).
Our goal is to identify characteristic sizes of these quantities, which must be functions of the nondimensional numbers identified in Table \eqref{nondim}.
...
These parameters are listed in Table \eqref{parameters}, and subtracting the ...
...
With this we may rewrite Equation \eqref{simple_milankovitch} in terms of the nondimensional numbers defined above.
...
\end{equation}
Plugging these scalings into equation \eqref{simple_milankovitch} we find
...
\begin{equation}
C_{ij} = \int_{V_S} \rho_0 \alpha (T-T_0) \left( r_q r_q \delta_{ij} - r_i r_j \right) 
...
The transform in Equation \eqref{eq:temperature_fluctuations} is picking out a degree-two spherical harmonic in laterally, and a degree-two polynomial radially.
...
The temperature field has been normalized by $\Delta T$ and thus goes between zero and one, therefore the expansion in $X_{lmn}$ is constrained by
...
Therefore we expect, to a good approximation, that we will be able to cut off the infinite sum in Equation \eqref{T_series} at some maximum wavenumber, set by the smallest lengthscale $d$:
\begin{equation}
...
The mismatch between the current axis and the principle axis of the convective moment is perhaps the most important parameter in Equation \eqref{scaled_rotation}.  
...
This is illustrated in Figure \eqref{fig:perturb}
...
There is a constant competition between growth of the mismatch angle $\theta$ through Equation \eqref{eq:grow_perturbation} and its relaxation through Equation \eqref{simple_milankovitch}.
We may plug in the estimate for $\Lambda_{ij}$ into equation \eqref{simple_milankovitch} to find the strikingly simple expression
\begin{equation}
