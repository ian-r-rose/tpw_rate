...
\begin{figure}
\label{misfit}
\includegraphics[]{figures/eigengap.pdf}
\caption{Oooga}
\end{figure}
...
We can expand the temperature structure of the convecting planet with a set of orthogonal basis functions $X_{lmn}$, 
where $l$, $m$, and $n$ are wavenumbers in the $x$, $y$, and $z$ directions, respectively.  
\begin{equation} 
T( x, y ,z )  = {\displaystyle \sum_{l m n} } T_{lmn} X_{lmn} (x , y, z)
\end{equation}

For chaotic thermal convection  there will be some power in every mode $X_{lmn}$ up to some wavenumber.  
At that critical wavenumber thermal diffusivity will 

\subsection{An estimate of the mismatch $\theta$}

\citet{davis1970rotation}

\begin{figure}
\label{misfit}
\includegraphics[width=10cm]{figures/misfit.pdf}
\caption{Oooga}
\end{figure}
