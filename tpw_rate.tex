...
\end{equation}
Equation \eqref{eq:quasistatic_liouville} indicates that ${\bf \Omega}$ and ${\bf H}$ are parallel, so a solution for ${\bf \Omega}(t)$ is equivalent to solving an eigenvalue problem for ${\bf I}(t)$, where the eigenvectors correspond to the principal axes and the eigenvalues correspond to the principal moments (where the most stable orientation of the planet corresponds to rotating about the largest principal axis).
...
We have recently implemented the ability to conduct simulations with a free surface in the CIG sponsored mantle convection software \texttt{ASPECT} \citep{rose2015free}.
...
We want to estimate the power in a single mode of the temperature expansion, allowing us identify
\begin{equation}
\Lambda_{31} \sim \Gamma T_{\mathrm{degree \textendash two}}
\end{equation}
...
This spectrum may or may not have a lot of power in degree-two, and does not depend strongly on time.
...
\begin{equation}
T_{\mathrm{degree \textendash two}}(t) \sim \frac{1}{N_{\mathrm{modes}}} \sim \left( \frac{d}{R}\right)^3
\end{equation}
...
\begin{equation}
T_{\mathrm{degree \textendash two}}(t) \sim \mathrm{Ra}^{-1}
...
We used the mantle convection software \texttt{ASPECT} \citep{kronbichler2012high}, based on the finite element library \texttt{deal.II} \citep{dealII81}.
which allows for flexible implementation of 
...
so $N_\mathrm{modes} \sim \left(R/d \right)^2$.  This leads us to a scaling of $T_{\mathrm{degree\textendash two}} \sim \mathrm{Ra}^{-2/3}$, which is used in the simulations shown in Figure~\ref{fig:eigengap}.
...
\label{eq:inertia}
\end{equation}
where $\bf r$ is the Eulerian coordinate, $\rho$ is the density, and $V$ is the volume of the material.  
...
\end{bmatrix}
\end{equation}
where $\bf 1$ is the identity matrix, and $\lambda_1$, $\lambda_2$, and $\lambda_3$ are the principal moments.
...
When density is a function of temperature, then we may directly identify this with the same expansion of the temperature field.
We can insert the equation of state~\eqref{eq:eos} into Equation~\eqref{eq:degree_two_moments} and integrate over a reference spherical volume $V_S$.
...
\end{aligned}
\end{equation}
We would like to normalize the differences in eigenvalues by the reference momentn $I_0$,
which can also be written in terms of real spherical harmonics:
\ifdetail
\begin{equation}
\begin{aligned}
...
\end{equation}
with this we can write expressions for $\Lambda_{ij}$:
\begin{equation}
\begin{aligned}
