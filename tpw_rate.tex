...
By definition, the rotaiton of the body-fixed frame relative to the inertial frame is specified by $\mitbf{\Omega}$.
...
The difference in the polar and equatorial moments of the hydrostatic planet $(C-A)$ is proportional to the ratio of rotational to gravitational forces(\citep{munk1960rotation}:
\begin{equation}
(C-A) \sim I_0 m.
\end{equation}
...
The direction of $\Psi$ is given by colatitude $\beta$ and longitude $\gamma$, as shown in Figure~\ref{fig:reference_frames}.

We may solve Equation~\eqref{eq:liouville_ode_new_frame} for $\dot{\theta}$ 
(i.e., the colatitudinal velocity in the $\mathbf{E}$ frame, not the TPW rate $\dot{\Theta}$)
in terms of $\theta$, $\beta$, and $\gamma$:
\begin{equation}
\dot{\theta} = 
...
(where we have neglected the orientation factor $\sin \beta \sin \gamma$ for the purposes of scaling).
...
Given a random perturbation to $\mathbf{E}$, we would like to give a bound on the size of the perturbation to $\theta$. 
