...
We want to estimate the power in those few modes, which we denote by $T_{\mathrm{degree \textendash two}}$: 
\begin{equation}
...
If the mismatch angle can become large, then we may estimate the maximum polar wander wander rate
\begin{equation}
...
\end{aligned}
\end{equation}
We would like to normalize the differences in eigenvalues by the reference moment $I_0$:
\ifdetail
\begin{equation}
\begin{aligned}
...
degree-two coefficients for the temperature field, which we abbreviate as $T_{\mathrm{degree \textendash two}}$:
\begin{equation}
\Lambda_{ij} \sim \Gamma T_{\mathrm{degree \textendash two}}
\end{equation}
