...
\centering
\label{misfit}
\includegraphics[width=20cm]{figures/eigengap.pdf}
\caption{Oooga}
\end{figure}
...
\centering
\label{misfit}
\includegraphics[width=10cm]{figures/perturb.pdf}
\caption{Oooga}
\end{figure}
...
\begin{equation} 
T( x, y ,z )  = {\displaystyle \sum_{l m n} } T_{lmn} X_{lmn} (x , y, z)
\end{equation}
...
\begin{equation}
\lVert T(x,y,z) \rVert_\infty = \lVert {\displaystyle \sum_{l m n} } T_{lmn} X_{lmn} (x , y, z) \rVert_\infty = 1
\end{equation}

This is a strong constraint, but it gives very little information about the distribution of power across the $T_{lmn}$.  We can, however, make hypotheses about the power spectrum in two different regimes: that of steady/quasisteady flow (relatively low $\mathrm{Ra}$) and that of chaotic flow (relatively high $\mathrm{Ra}$).
...
\begin{equation}
\sin(2 \theta) \le \frac{ 2 \lVert \delta \rVert}{ \displaystyle \min_{i \neq j} \lVert \lambda_i - \lambda_j \rVert }
\end{equation} 
...
Furthermore, the quantity $(\lambda_i-\lambda_j)$ which appears in the denominator is precisely the same as the quantity which we estimated in the previous section to be $\sim \mathrm{Ra}^{-1}$.
...
This result agrees with the qualitative results of 

\begin{figure}
\centering
\label{misfit}
\includegraphics[width=20cm]{figures/misfit.pdf}
\caption{Oooga}
\end{figure}
