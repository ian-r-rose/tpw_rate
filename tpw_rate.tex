...
However, the rate of true polar wander in Equation~\eqref{eq:polar_wander_rate_tensor} is a scalar quantity, which means it is invariant to rotations.
...
The second special case is if the rotation vector $\mitbf{\omega}$ lies in $\mathbf{e}_1$-$\mathbf{e}_2$ plane ($\phi = 0$).
...
This parameter is crucial for the rate of TPW, as described in is in Equation~\eqref{eq:scaled_rotation}.  
...
As in Equation~\eqref{eq:simple_milankovitch}, we can choose a convenient coordinate system by aligning the axes so that 
$\mitbf{\omega}^{\prime}$ lies in $\mathbf{e}_1^{\prime}-\mathbf{e}_3^{\prime} $ plane (as shown in Figure~\ref{fig:reference_frames}). 
In this case the orientation of $\mitbf{\omega}$ is defined by colatitude $\theta$ and longitude $\phi=0$. 
...
The orientation of $\Psi$ is not restricted in this frame, 
so we specify the direction using colatitude $\beta$ and longitude $\gamma$ (see Figure~\ref{fig:reference_frames}).
...
Whereas the prefactor $\mathrm{Ra}^{1-3\beta}$ in Equation~\eqref{eq:simplest_milankovitch} has a weak dependence on $\mathrm{Ra}$, 
