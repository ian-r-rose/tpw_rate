...
In Chapter~\ref{chap:free_surface} we describe the implementation of a free surface boundary condition in the CIG-sponsored mantle convection software \texttt{ASPECT}.
...
\section{Derivation of the polar wander rate equation}
\label{appendix:scalar_liouville}

Here is the tedious algebra for deriving Equation~\eqref{eq:polar_wander_rate}.  
We begin with the scalar equation Equation~\eqref{eq:polar_wander_rate_tensor}.
Let the prefactors $\frac{1}{(C-A)T_1}$ be denoted by $A$:

\begin{equation}
\begin{aligned}
\dot{\mitbf{\omega}} \cdot \dot{\mitbf{\omega}} = \dot{\Theta}^2  &= 
A^2 \left[ \left(\mathbf{E}\cdot \mitbf{\omega}\right)^2
 - \left(\mitbf{\omega} \cdot \mathbf{E} \cdot \mitbf{\omega} \right)^2.
\right]\\
\end{aligned}
...
\end{bmatrix}
\end{equation}
We can evaluate some of the terms of the scalar equation for $\dot{\Theta}^2$:
\begin{equation}
\mathbf{E}\cdot \mitbf{\omega} = 
\begin{bmatrix}
\lambda_1 \sin{\theta} \\
...
\end{aligned}
\end{equation}
which is equivalent to Equation~\eqref{eq:simple_milankovitch}.
...
\end{aligned}
\end{equation}
Again, plugging these into the scalar equation for $\dot{\Theta}^2$, we find:
\begin{equation}
\begin{aligned}
\frac{\dot{\Theta}^2}{A^2}  &= 
 \left(\mathbf{E}\cdot \mitbf{\omega}\right)^2
 - \left(\mitbf{\omega} \cdot \mathbf{E} \cdot \mitbf{\omega} \right)^2  \\
&=
\lambda_1^2 \sin^2{\theta} \cos^2{\phi} + \lambda_2^2 \sin^2{\theta} \sin^2{\phi} + \lambda_3^2 \cos^2{\theta} \\
...
\end{aligned}
\end{equation}
After a lot of simplification, we get
\begin{equation}
\begin{aligned}
\frac{\dot{\Theta}^2}{A^2}  = 
&\frac{1}{4}\sin^2{2\theta} \left[( \lambda_3 - \lambda_1)^2 \cos^2{\phi} + (\lambda_3 - \lambda_2)^2 \sin^2{\phi} \right] +\\
...
\end{aligned}
\end{equation}
which is Equation~\eqref{eq:polar_wander_rate}.
